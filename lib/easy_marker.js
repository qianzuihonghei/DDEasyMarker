import DefaultCursor, { CursorType } from './element/cursor'
...
class EasyMarker {
  ...
  create(containerElement, scrollContainerElement, excludeElements = []) {
    ...
    const Cursor = this.options.cursor && this.options.cursor.customClass
      ? this.options.cursor.customClass : DefaultCursor

    if (this.options.cursor.same) {
      this.cursor.start = new Cursor(this.container, CursorType.END, this.options.cursor || {})
    } else {
      this.cursor.start = new Cursor(this.container, CursorType.START, this.options.cursor || {})
    }
    this.cursor.end = new Cursor(this.container, CursorType.END, this.options.cursor || {})
    this.movingCursor = this.cursor.end

    this.mask = new Mask(this.container, this.options.mask || {})
    this.highlight = new Highlight(this.container, this.options.highlight || {})
  ...
  getSelectMarkdown() {
    return this.markdown.getSelectMarkdown(
   ...
   */
  isContains(element) {
    return this.container.contains(element) && this.excludeElements.findIndex(el => el.contains(element)) === -1
  }
  ...
  handleTap(e) {
    if (this.selectStatus === SelectStatus.FINISH) {
      ...
      if (!inHighlightLine && !this.options.disableTapHighlight && this.isContains(e.target)) {
